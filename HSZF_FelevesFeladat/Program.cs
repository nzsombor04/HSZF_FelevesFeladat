    ...
    internal class Program
    {
        ...
        public List<Employee> XmlLoad()
        {
            ...
            var employees = xmlDoc.Descendants("Employee")
                ...
                {
                    int commission = 0;
                    if (e.Element("Commission") != null)
                    {
                        var commissionElement = e.Element("Commission");
                        var currency = commissionElement.Element("currency")?.Value;
                        if (currency == "eur")
                        {
                            commission = int.Parse(commissionElement.Value) * 400;
                        }
                        else
                        {
                            commission = int.Parse(commissionElement.Value);
                        }
                    }
